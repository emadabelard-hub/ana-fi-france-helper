...
function extractedDataHasForbiddenScripts(data: ExtractedData): boolean {
  const texts: Array<string | undefined> = [
    data.clientName,
    data.clientAddress,
    data.workSiteAddress,
    data.notes,
    ...data.items.flatMap((it) => [it.designation_fr, it.designation_ar]),
  ];
  return texts.some((t) => (t ? hasForbiddenScripts(t) : false));
}
...
serve(async (req) => {
  ...
  try {
    ...
    const baseSystemPrompt = `Tu es un assistant expert en extraction de données de documents commerciaux français (devis, factures).
...
- Interdiction stricte d'utiliser un alphabet autre que latin (français) ou arabe.
- Interdiction: cyrillique, grec, chinois, japonais, coréen, hébreu, etc.
- Si tu dévies, régénère et corrige.`;
...
IMPORTANT: Traduis tout texte arabe en français professionnel.`;
    ...
    // First attempt
    let extractedData = await callAI(baseSystemPrompt);

    // Retry if forbidden scripts detected
    if (extractedDataHasForbiddenScripts(extractedData)) {
      console.warn(`[${VERSION}] Forbidden scripts detected in extracted data. Retrying...`);
      extractedData = await callAI(baseSystemPrompt + strictAlphabetRule);
    }

    if (extractedDataHasForbiddenScripts(extractedData)) {
      throw new Error("Caractères non autorisés détectés dans la sortie. Réessayez.");
    }
